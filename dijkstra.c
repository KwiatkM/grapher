...
static int dijTabInit (int wymiar_x, int wymiar_y){     
    int i;
    poprzednik = malloc(wymiar_x * wymiar_y * sizeof(*poprzednik));
    odleglosc = malloc(x * y * sizeof(*odleglosc));                     
    odwiedzono = malloc(x * y * sizeof(*odwiedzono));

    if(poprzednik == NULL || odleglosc == NULL || odwiedzono == NULL)    //jesli nie udalo sie zalokowac pamieci dla conajmniej jednej z tablic
        return 1;

    for(i = 0; i < wymiar_x * wymiar_y; i++){
        poprzednik[i] = -1;         
        odleglosc[i] = INFINITY;            //ustawiamy domyslne wartosci w tablicach
        odwiedzono[i]= '0';
    }
    return 0;               //gdy udalo sie zainicjowac tablice
}

static int pKolejkaDodaj (pkolejka_t ** kolejka, int nr_wierz){
    pkolejka_t * tmp = malloc(1 * sizeof(*tmp));
    tmp->nr_wierz = nr_wierz;
    tmp->next = NULL;
    tmp->prev = NULL;

    if(*kolejka == NULL){           //jesli kolejka jest pusta
        *kolejka = tmp;
        return 0;
    }
    ...
    while(head != NULL){
        if(odleglosc[nr_wierz] < odleglosc[head->nr_wierz]){ 
            ...
            tmp->prev = head->prev;                         
            if(head->prev != NULL)
                head->prev->next = tmp; //to samo co head = tmp(???)
            head->prev = tmp;
            if(odleglosc[(*kolejka)->nr_wierz] > odleglosc[tmp->nr_wierz]) 
            *kolejka = tmp;                                     // gdy wkładany element jest mniejszy od elementu na
...
static int pKolejkaZdejmij (pkolejka_t ** kolejka){
    if(*kolejka == NULL)                //jesli pusta kolejka
        return -1;
    int tmp = (*kolejka)->nr_wierz;     //przechowujemy uchwyt i jego nr wierz
    pkolejka_t * tmp_k = *kolejka;
    *kolejka = (*kolejka)->next;        //przesuwamy wskaznik o jedno miejsce
    if(*kolejka != NULL)                //jesli nie osiagnelismy konca kolejki
        (*kolejka)->prev = NULL;
    free(tmp_k);
    return tmp;
}
...
if(odleglosc[head->prev->nr_wierz] < odleglosc[head->nr_wierz])
    return 0;
            ...
            if((*graf+u-1)->right > 0){ // jeżli istnieje przejście
                if(odwiedzono[u-1] == '0'){ // jeżeli u nie był odwiedzony
                    if(odleglosc[u-1] > odleglosc[u] + (*graf+u-1)->right ){
                        ...
                        poprzednik[u-1] = u;
                        if(tmp = INFINITY)
                            pKolejkaDodaj (&kolejka, u-1);
                        else
                            pKolejkaAktualizuj (&kolejka, u-1);
                    }
                }
            }
        }
            ...
            if((*graf+u-y)->down > 0){
                if(odwiedzono[u-y] == '0'){
                    if(odleglosc[u-y] > odleglosc[u] + (*graf+u-y)->down){
                        ...
                        poprzednik[u-y] = u;
                        if(tmp = INFINITY)
                            pKolejkaDodaj (&kolejka, u-y);
                        else
                            pKolejkaAktualizuj (&kolejka, u-y);
                    }
                }
            }
        }
...
int wypisz_sciezke(FILE * out, int w_start, int w_konc){
    ...
    int w = poprzednik[w_konc];
    fprintf(out, "%d ", w_konc);
    while (w != -1){
        fprintf(out, "-> %d ", w);
        w = poprzednik[w];
        if(n == 10){
